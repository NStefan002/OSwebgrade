#include <stdio.h>
#include <stdlib.h>
#include <ctype.h>
#include <sys/stat.h>
#include <sys/types.h>
#include <fcntl.h>
#include <unistd.h>

#define check_error(cond, msg)\
    do\
    {\
        if (!(cond))\
        {\
            perror(msg);\
            fprintf(stderr, "file: %s\nfunc: %s\nline: %d\n", __FILE__, __func__, __LINE__);\
            exit(EXIT_FAILURE);\
        }\
    } while (0)


void changeCases(const char *fpath, off_t p, size_t n);
void swapCases(char *str);

int main(int argc, char **argv)
{
    check_error(argc == 4, "./3 file p n");    

    changeCases(argv[1], strtol(argv[2], NULL, 10), strtol(argv[3], NULL, 10));

    return 0;
}

void swapCases(char *str)
{
    for (int i = 0; str[i]; i++)
    {
        if (islower(str[i]))
        {
            str[i] = toupper(str[i]);
        }
        else if (isupper(str[i]))
        {
            str[i] = tolower(str[i]);
        }
    }
}

void changeCases(const char *fpath, off_t p, size_t n)
{
    struct stat file_info;
    check_error(stat(fpath, &file_info) != -1, "stat");
    check_error(S_ISREG(file_info.st_mode), "not a reg file");

    int fd;
    check_error((fd = open(fpath, O_RDWR)) != -1, "open");
    
    check_error(lseek(fd, p, SEEK_SET) != -1, "lseek");
    char *mem_buf = malloc((n + 1) * sizeof(char));
    check_error(mem_buf != NULL, "malloc");

    ssize_t read_bytes = read(fd, mem_buf, n);
    check_error(read_bytes != -1, "read");
    mem_buf[read_bytes] = '\0';

    swapCases(mem_buf);

    check_error(lseek(fd, p, SEEK_SET) != -1, "lseek");
    check_error(write(fd, mem_buf, read_bytes) != -1, "write");

    check_error(close(fd) != -1, "close");
    free(mem_buf);
}